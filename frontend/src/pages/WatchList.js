    ...
    useEffect(() => {
        if (!user) {
            ...
            if (!isActive) {
                ...
                .then((data) => {
                    ...
                    fetch(host + '61e98b8eeb5ad0d3ad9338ddfe0ee35b7535929e9a6084fac92742858e1bba84/', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({username: data['username'], target: "watch_list"})
                    })
                    .then(response => {
            ...
            fetch(host + '61e98b8eeb5ad0d3ad9338ddfe0ee35b7535929e9a6084fac92742858e1bba84/', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({username: user['username'], target: "watch_list"})
            })
            .then(response => {
                if (response.status === 200) {
                    return response.json();
    ...
    if (user){
        return (
            <div className="full-container-dashboard">
                ...
                <h1 className="dashboard-title">Bookmarked By {user.fname}</h1>
